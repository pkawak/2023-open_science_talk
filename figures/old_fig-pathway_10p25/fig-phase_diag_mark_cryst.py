...
#ax.text(0.04, 0.17, 'Nematic', weight='bold', color='g', fontsize=8, fontname='dejavusans', transform = ax.transAxes)
...
#ax.text(0.68, 0.35 , 'Melt'   , weight='bold', color='r', fontsize=8, fontname='dejavusans', transform = ax.transAxes)

ax.fill_between(T_quad, phi_min, phi_quad, color=['#EFEFEF'], alpha=0.5)
#ax.text(0.21, 0.71, 'Crystal', weight='bold', color='b', fontsize=8, fontname='dejavusans', transform = ax.transAxes)
...
ax.scatter(T_trans, phi_trans, s=30, c='b')
t = ax.text(T_trans-.3, phi_trans+0.03, "("+str(T_trans)+","+str(phi_trans)+")", fontsize=8, color='w')
t.set_bbox(dict(facecolor='b', alpha=0.5))
plt.subplots_adjust(left=0.11, top=1.0, bottom=0.11, right=1.0)
#plt.show()
#sys.exit()
