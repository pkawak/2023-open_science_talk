...
ax.tick_params(axis='both', labelsize=labelsize-2, pad=0)
ax.scatter(step_kb1_kappa1_lp_df["Tr"]   , step_kb1_kappa1_lp_df["lp"]   , s=5, color='m', label='discrete')
ax.scatter(harm_kb1_kappa1_lp_df["Tr"]   , harm_kb1_kappa1_lp_df["lp"]   , s=5, color='b', label='continuous')
...
legend_elements = [
                   Line2D([0], [0], marker='+', color='k', markeredgewidth=1.5, label=r'$\epsilon_{l}^{*} = 600$',
                          markerfacecolor='k', markersize=5),# handlelength=1),
                   Line2D([0], [0], marker='*', color='w', label=r'$\epsilon_{\theta}^{*} = 20$',
...
ax.legend(loc='best', ncol=2, handles=legend_elements, handlelength=0.5, columnspacing=1.2, handletextpad=0.3, fontsize=labelsize)
...
if test == 0:
  plt.subplots_adjust(left=0.139, top=0.983, bottom=0.135, right=0.994)

left, bottom, width, height = [0.5, 0.28, 0.45, 0.45]
