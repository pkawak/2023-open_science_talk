...
# plot total energy
test = 0
fig, ax = plt.subplots(figsize=(4.25, 2.80), constrained_layout=test)
...
ax.set_xlim(sweeps_orig[0]-200, sweeps_orig[-1]+200)
ax.hlines(Emean_tot, ax.set_xlim()[0], ax.set_xlim()[1], colors='black', linestyles='--', zorder=2, label=r'$\mu(U)$')
...
if test == 0:
  plt.subplots_adjust(left=0.064, bottom=0.065, right=0.995, top=0.998)
from mpl_toolkits.axes_grid1.inset_locator import (inset_axes, InsetPosition,
                                                  mark_inset)
ax3 = plt.axes([0,0,1.45,0.68])
ip = InsetPosition(ax3, [0.11,0.51,0.70,0.75])
...
rangeh = rangel+13
#for i in range(13):
#  print(sweeps_orig[rangel+i], etot_orig[rangel+i])
...
ax.set_xticks([])
ax.set_xlabel(r'$t$', fontsize=10, labelpad=2.7)
ax.set_ylabel(r'$U(t)$', fontsize=10, labelpad=-10)
#ax.legend(loc='best')
...
ax.vlines(tau, np.amin(acf_tot), np.amax(acf_tot), color='g', ls='--', label=r'$\tau=$'+str(int(tau))+'='+str(round(tau_time))+' seconds')
...
ax.set_ylabel(r'$\mathrm{acf}\left(\Delta t\right) = \mathrm{acf}\left(U\left(t+\Delta t\right), U\left(t\right)\right)$', fontsize=10)#, labelpad=-3)
ax.tick_params(axis='both', labelsize=10, pad=0)
ax.legend(loc='best', fontsize=10)
ax.set_ylim(min(0, np.amin(acf_tot)), 1)
ax.set_xlim(0, num_tot[-1])
#plt.show()
if test == 0:
  plt.subplots_adjust(left=0.106, bottom=0.076, right=0.995, top=0.998)
plt.savefig('subfig-acf_sub.pdf');
plt.close()
