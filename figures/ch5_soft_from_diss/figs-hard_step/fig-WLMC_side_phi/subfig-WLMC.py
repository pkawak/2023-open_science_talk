...
WL_A_file_name   = "lx11.715077_ext_canon_distro.out"
...
Nbond = Nc*(Nb-2)
#pu, = ax[0].plot(WL_A_df["T"], (WL_A_df["Em"]+Nbond)/Nbond, c=plt.get_cmap('tab10')(0), label=r"$\langle U \rangle_{\mathrm{norm}}$")
pp, = ax[0].plot(WL_A_df["T"], WL_A_df["P2"]              , c=plt.get_cmap('tab10')(0), label=r"$P_{2}$")
pq, = ax[0].plot(WL_A_df["T"], WL_A_df["n6"]              , c=plt.get_cmap('tab10')(2), label=r"$f_{\mathrm{cryst}}$")
ax2 = ax[0].twinx()
pc, = ax2.plot(WL_A_df["T"]  , WL_A_df["Cv"]/Nc/(Nb-2)    , c=plt.get_cmap('tab10')(3), label=r"$C_{V}/kN_{c}$")
ax2ylim1 = 0.5
ax2ylim2 = 250
...
ax2.set_ylabel(r"$C_{V}/kN_{\mathrm{bonds}}$", fontsize=10, color=plt.get_cmap('tab10')(3), labelpad=0)
...
Nbond = Nc*(Nb-2)
#pu, = ax[1].plot(WL_B_df["T"], (WL_B_df["Em"]+Nbond)/Nbond, c=plt.get_cmap('tab10')(0), label=r"$\langle U \rangle_{\mathrm{norm}}$")
pp, = ax[1].plot(WL_B_df["T"], WL_B_df["P2"]              , c=plt.get_cmap('tab10')(0), label=r"$P_{2}$")
pq, = ax[1].plot(WL_B_df["T"], WL_B_df["n6"]              , c=plt.get_cmap('tab10')(2), label=r"$f_{\mathrm{cryst}}$")
ax2 = ax[1].twinx()
pc, = ax2.plot(WL_B_df["T"]  , WL_B_df["Cv"]/Nc/(Nb-2)    , c=plt.get_cmap('tab10')(3), label=r"$C_{V}/kN_{c}$")
ax2.set_ylim(ax2ylim1, ax2ylim2)
ax2.tick_params(axis='y', labelsize=8, labelcolor=plt.get_cmap('tab10')(3), pad=0)
ax2.set_yscale('log')
#ax2.set_ylabel(r"$\frac{C_{V}}{kN_{\mathrm{bonds}}}$", fontsize=12, color=plt.get_cmap('tab10')(3), labelpad=-3)
