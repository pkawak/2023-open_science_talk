...
WL_A_file = "../Lx15.000_kb0.10_WLMC_Ts.out"
WL_B_file = "../Lx15.000_kb1.00_WLMC_Ts.out"
WL_C_file = "../Lx15.000_kb10.0_WLMC_Ts.out"
Lx_A      = 15.000
Lx_B      = 15.000
Lx_C      = 15.000
...
T_B = peak_B = E_B = []
T_C = peak_C = E_C = []
Ts_data_A = np.loadtxt(WL_A_file, skiprows=1, ndmin=2)
if len(Ts_data_A) > 0:
  T_A       = Ts_data_A[:,0]
  T_A       /= kb_A
  peak_A    = Ts_data_A[:,1]
...
Ts_data_B = np.loadtxt(WL_B_file, skiprows=1, ndmin=2)
if len(Ts_data_B) > 0:
  T_B       = Ts_data_B[:,0]
  T_B       /= kb_B
  peak_B    = Ts_data_B[:,1]
