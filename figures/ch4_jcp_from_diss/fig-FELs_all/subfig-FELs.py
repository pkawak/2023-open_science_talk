...
#  ax.set_title(name, fontsize=10, pad=30.0)
  ...
  if j == 0:
    ...
    if ii == 0:
      plt.subplots_adjust(left=0.249, bottom=0.07, right=0.933, top=1.00)
    else:
      plt.subplots_adjust(left=0.25, bottom=0.07, right=0.935, top=1.00)
  else:
    divider = make_axes_locatable(ax)
    ...
    cbar.ax.tick_params(labelsize=8)
    if ii == 0:
      plt.subplots_adjust(left=0.195, bottom=0.115, right=0.915, top=1.00)
    else:
      plt.subplots_adjust(left=0.195, bottom=0.115, right=0.91, top=1.00)
  plt.savefig("subfig-FEL_"+names_alt[i]+".pdf")
#  if i == 4:
#    plt.show()
#    sys.exit()
#  plt.close()
