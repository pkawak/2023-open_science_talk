...
#  print(np.amin(lng), np.amax(lng))
  ...
  scaled = [ (yyy-min_extent)/(max_extent-min_extent) for yyy in y_label_list ]
  y_label_list_big = [ round((max_extent-min_extent)/5*(i), 2) for i in range(6) ]
  for ijk in range(len(point_j)):
    point_j[ijk] = ( (int(point_j[ijk]) + start_/5) )
  im = ax.imshow(lng_2d, interpolation='bilinear', cmap=cm.get_cmap('coolwarm', 20),
                    vmin=vmin, vmax=vmax,
                    origin='lower', extent=[0,1,scaled[0],scaled[-1]])#,
...
#  if i == 0:
  ax.set_yticklabels(y_label_list_big, fontsize=8)
#  else:
#    ax.set_yticklabels([])
...
#  ax.set_title(name, fontsize=10, pad=30.0)
  for iii in range(len(point_i)):
    ax.scatter((float(point_i[iii])+0.5)/np.shape(lng_2d)[1], (float(point_j[iii])+0.5)/np.shape(lng_2d)[0]*(scaled[-1]-scaled[0])+scaled[0], c='k', s=20, marker='x')
  if j == 0:
  ...
  plt.savefig("subfig-FEL_"+names_alt[i]+"_sim_domain.pdf")
#  if i == 4:
#    plt.show()
#    sys.exit()
#  plt.close()
