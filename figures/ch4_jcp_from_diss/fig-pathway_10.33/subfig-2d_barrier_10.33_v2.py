...
with open("heights.out", "w") as f:
  f.write(str(np.amin(y[0:id_max])) + " " + str(np.amax(y)) + " " + str(y[-1]))
...
ax.set_ylabel(r'$\Delta F/\epsilon$', fontsize=10, labelpad=7)
...
plt.subplots_adjust(left=0.13, bottom=0.21, right=0.99, top=0.99)
#plt.legend(loc='best', prop={'size': 8})
##plt.show()
fig.savefig("subfig-2d_barrier_10.33_v2.pdf")
