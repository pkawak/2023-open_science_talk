...
ax[0].axvline(x=0.2633, color='b', alpha=0.5, ls=':', label=r'$T_{r}=0.263$')
...
#ydel=0.2
#yrange = np.arange(max(round(min(WL_df["P2"]), 1)-ydel, 0), min(round(max(WL_df["P2"])+ydel, 1), 1+ydel), ydel)
#print(yrange)
#ax[1].set_yticks(yrange)
ax[1].set_ylabel(r"$P_{2}$", fontsize=10, labelpad=11)
ax[1].tick_params(axis='both', labelsize=8)
ax[1].axvline(x=0.2633, color='b', alpha=0.5, ls=':')
...
ax[2].axvline(x=0.2633, color='b', alpha=0.5, ls=':')
ax[2].set_xlabel(r"$T_{r}$", fontsize=10, labelpad=5)
#ax[2].set_yticks([0.43, 0.44, 0.45, 0.46])
ydel = 0.01
yrange = np.arange(round(min(WL_df["q6"]), 2)-ydel, round(max(WL_df["q6"]), 2), ydel)
ax[2].set_yticks(yrange[1:])
...
plt.savefig("fig-WLMC_plus_MCMC_10.33.pdf")
