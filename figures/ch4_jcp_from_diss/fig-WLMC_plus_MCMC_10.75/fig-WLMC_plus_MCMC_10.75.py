...
ax[0].axvline(x=0.2262, color='b', alpha=0.5, ls=':', label=r'$T_{r}=0.226$')
...
#yrange = np.arange(round(min(WL_df["P2"]), 1)-0.1, round(max(WL_df["P2"]), 1)+0.1, 0.1)
#print(yrange)
#ax[1].set_yticks(yrange)
ax[1].set_ylabel(r"$P_{2}$", fontsize=10, labelpad=11)
ax[1].tick_params(axis='both', labelsize=8)
ax[1].axvline(x=0.2262, color='b', alpha=0.5, ls=':')
...
ax[2].axvline(x=0.2262, color='b', alpha=0.5, ls=':')
...
plt.subplots_adjust(left=0.2, top=0.975, bottom=0.07, right=0.995, hspace=0.1)
plt.savefig("fig-WLMC_plus_MCMC_10.75.pdf")
